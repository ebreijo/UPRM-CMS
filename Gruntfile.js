...
module.exports = function(grunt) {
    ...
    exec: {
      createDb: {
        cmd: function() {
          ...
          var databaseName = 'uprm_cms_' + env;
          var result = _.template(template, {'databaseName': databaseName});

          fs.writeFileSync(__dirname + '/database/schema.sql', result);

          return 'mysql -u root < database/schema.sql && mysql -u root ' + databaseName + ' < test/server/seedData/populateDB.sql';
        }
      }
    }
  });
  ...
  grunt.registerTask('createDb', ['exec:createDb', 'clean:schema']);
