...
module.exports = function(router) {
  ...
  router.get('/api/admins/companiesJobFair', admins.getAllCompaniesJobFair);
   ...
   */
  router.get('/api/admins/companies/:companyId/promotionalMaterial', admins.getCompanyPromotionalMaterialForAdmins);
  // Get PromID Parameter.
  router.param('adminPromId', admins.getAdminPromId);
  // Edit a Promotional Material of a Specific Company.
  router.put('/api/admins/companies/:companyId/promotionalMaterial/:adminPromId', admins.editCompanyPromotionalMaterialForAdmins);
  // Delete a Promotional Material of a Specific Company.
  router.delete('/api/admins/companies/:companyId/promotionalMaterial/:adminPromId', admins.deleteCompanyPromotionalMaterialForAdmins);

  /**
   * Company Job Offer Routes for admins.
   */
    // Get All Job Offers from a given company
  router.get('/api/admins/companies/:companyId/jobOffers', admins.getAllCompanyJobOffersForAdmins);
  // Get JobID Parameter.
  router.param('adminJobId', admins.getAdminJobOfferId);
  // Get Job Offer by Job Offer ID.
  router.get('/api/admins/companies/:companyId/jobOffers/:adminJobId', admins.getCompanyJobOfferForAdmins);
  // Remove/disable a job offer for a given job offer id
  router.put('/api/admins/companies/:companyId/jobOffers/:adminJobId', admins.editCompanyJobOfferForAdmins);
  ...
  router.put('/api/admins/companies/:companyId/recruiters', admins.updateRecruiterStatus);
