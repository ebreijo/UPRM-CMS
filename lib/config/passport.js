...
/**
 * Create a strategy to authenticate an administrator. If the administrator has access (active), then it can login.
 */
passport.use('local-admin', new LocalStrategy({
  ...
  function (email, password, done) {
    ...
    AdministratorAccess.find({
      ...
      if(adminAccess) {
        return Administrator.find({where: {email: email}});
      } else {
...
/**
 * Create a strategy to authenticate a recruiter. If the recruiter is active, then it can login.
 */
