          ...
          fs.exists(path, function(exists) {
            if (exists) {
              fs.unlink(path, function(err) {
                ...
                console.log('File ' + jobOffer.flyerPath + ' deleted successfully!');
              });
            } else {
              console.log('No file exists in ' + jobOffer.flyerPath);
            }
          });
        ...
        if (promMaterial.expirationDate <= (new Date())) {
          ...
          fs.exists(path, function(exists) {
            if (exists) {
              ...
              console.log('File ' + promMaterial.filePath + ' deleted successfully!');
            }
          });

          db.promotionalMaterial.update(promMaterial.dataValues, {
            where: {
              id: promId
            }
          }).catch(function(err) {
