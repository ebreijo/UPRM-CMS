...
exports.authRecruiterOrStudentOrAdmin = auth(function(user) {
  return authType.isRecruiter(user) || authType.isAdmin(user) || authType.isStudent(user);
});

exports.authStudentOrAdmin = auth(function(user) {
  return authType.isStudent(user) || authType.isAdmin(user);
});
...
/**
 * Clean up every day any uploaded promotional material which file path is not stored in the database.
 */
setInterval(function() {
  // Path for Promotional Material
  var promoMaterialPath = __dirname + '/../app/media/promotionalMaterial/';

  // Read the promotional material filenames
  fs.readdir(promoMaterialPath, function(err, filenames) {
    if (err) {
      console.log(err);
    } else {
      db.promotionalMaterial.findAll({
        ...
        var promotionalMaterialFilePathMap = _.groupBy(results, function(data) { return path.basename(data.dataValues.filePath); });

        // For each company attending the job fair, verify the positions it is looking for and append it to the end
        _.forEach(filenames, function(filename) {
          if (!promotionalMaterialFilePathMap[filename]) {
            fs.unlink(promoMaterialPath + filename, function (err) {
              if (err) {
                return console.error(err);
              }
              console.log('Promotional Material file ' + filename + ' deleted successfully!');
            });
          }
        });
      }).catch(function(err) {
