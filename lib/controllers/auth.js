...
exports.validateStudent = function(req, res, next) {
  ...
  if (!req.query.ticket) {
    return next(new errors.Http401Error('Not authorized'));
  }

  db.students.create({studentTicket: req.query.ticket}).then(function() {
    res.redirect('http://' + req.get('host') + '/student?ticket=' + req.query.ticket);
