...
var logoUpload = multer({
  dest: path.resolve(__dirname + '/../../app/media/companyLogos/'),
  limits: {
    fileSize: 1000000,
    files: 1
  }
...
var jobOfferUpload = multer({
  dest: path.resolve(__dirname + '/../../app/media/jobOffersPDFs/'),
  limits: {
    fileSize: 1000000,
    files: 1
  }
...
var promotionalMaterialUpload = multer({
  dest: path.resolve(__dirname + '/../../app/media/promotionalMaterialPDFs/'),
  limits: {
    fileSize: 1000000,
    files: 1
  }
...
exports.register = function(req, res, next) {
  ...
  var passwordRegex = new RegExp(/^(?=(.*[0-9]))(?=.*[\!@#$%^&*()\\[\]{}\-_+=~`|:;'<>,.?])(?=.*[a-z])(?=(.*[A-Z]))(?=(.*)).{6,}$/);
  ...
  if (companyInfo) {
    ...
    return db.companies.findById(recruiterInfo.companyName).then(function (data) {
      if (data) {
