...
exports.logoCleanup = function(req, res, next) {
  console.log('*** Logo Cleanup Start ***');
  var receivedLogoName = req.body.logoPath.replace('media/companyLogos/', '');

  // Path for Company Logos:
  var logosPath = path.resolve(__dirname + '/../../app/media/companyLogos/');
  var logos = [];
  // Store all logo names in a single array.
  fs.readdirSync(logosPath).forEach(function(file) {
    if(file !== receivedLogoName){
      logos.push(file);
    }
  });
  ...
  db.companies.findAll({
    ...
    data.forEach(function(logo){
      if(logo.logoPath !== null){
        var logoPath = logo.logoPath.replace('media/companyLogos/', '');
        dbLogos.push(logoPath);
      }
    });

    // If no logos are stored on database, delete all logos on file system.
    if(dbLogos.length === 0){
      logos.forEach(function(logo){
        fs.unlinkSync(logosPath + '/' + logo);
      });
    } else { // Logos exist on database.
      logos.forEach(function(logo){
        var contained = false;
    ...
    res.send('Logo Cleanup Complete!');
