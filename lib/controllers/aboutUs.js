...
/**
 * Get a requirement given its id
 * @param req
 * @param res
 * @param next
 * @param requirementId
 */
exports.getRequirementId = function(req, res, next, requirementId) {
  db.requirements.findById(requirementId).then(function(data) {
    if (data) {
      req.requirement = data;
      return next();
    } else {
      return next(new errors.Http404Error('Policy not found'));
    }
  }).catch(function(err) {
    next(new errors.DatabaseError(err.message));
  });
 ...
 * @param compServiceId
 */
exports.getCompanyServiceId = function(req, res, next, compServiceId) {
  db.companyServices.findById(compServiceId).then(function(data) {
    if (data) {
      req.companyService = data;
      return next();
    } else {
      return next(new errors.Http404Error('Policy not found'));
    }
 ...
 * @param policyId
 */
exports.getPolicyId = function(req, res, next, policyId) {
  db.policies.findById(policyId).then(function(data) {
    if (data) {
      req.policy = data;
      return next();
    } else {
 ...
 * @param staffId
 */
exports.getStaffId = function(req, res, next, staffId) {
  db.ourStaff.findById(staffId).then(function(data) {
    if (data) {
      req.staff = data;
      return next();
    } else {
      return next(new errors.Http404Error('Staff not found'));
    }
  }).catch(function(err) {
    next(new errors.DatabaseError(err.message));
  });
 ...
 * @param servId
 */
exports.getStudentServiceId = function(req, res, next, servId) {
  db.studentServices.findById(servId).then(function(data) {
    if (data) {
      req.studentService = data;
      return next();
    } else {
      return next(new errors.Http404Error('Student Service not found'));
    }
  }).catch(function(err) {
    next(new errors.DatabaseError(err.message));
  });
